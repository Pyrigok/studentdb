	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			try:
				...
				message = u'Під час відправки листа виникла помилка. Спробуйте скористатись даною формою пізніше.'
			else:
				message = u'Повідомлення успішно надіслане!'

			# redirect to same contact page with succes message
			return HttpResponseRedirect (u'%s?status_message=%s' % (reverse('contact_admin'), message))

	# if thete was not POST render blank form
	else:
		form = ContactForm ()
